...
# Author: Christoph Hesse (April 2021)
# 3-strategy Nash demand game with dynamic disagreement point updates
# and dynamic updates of beliefs about disagreement points
# Based on the Nash demand game (1950) and inspired by
# Travis LaCroix & Cailin O'Connor (2018)
# =====================================================================

# Being the victim of slurs makes you feel like you have less support, and less support means less bargaining power (but indirectly via belief updates).
# An agent who's been insulted, believes their disagreement point is less than what it actually is,
# so they should make lower bids than they would make otherwise (so it affects expected utility, but not actual utility).
# However, their actual payoffs will be based on their actual disagreement point (which creates a feedback loop to their future behaviour).

# we initialize all agents with two copies of the payoff matrix: one for beliefs about payoffs and one for actual payoffs
# slurring affects the disagreement point in the believed payoffs, not the disagreement point in the actual payoffs
# agents choose their best response using their beliefs

# There is an audience witnessing each interaction. The audience acts as a jury and can directly affect agents' actual bargaining power.
# Slurs only indirectly affect agents' bargaining power through modification of beliefs leading to behaviour that reflects less bargaining power.
...
# 10) a copy of the payoff matrix representing agents' belief about their expected payoffs
...
class par:
	...
	N1 = 1#N/2
	N2 = 5#N/2
	N3 = 4# powerful individuals / racists
	...
	# groups' propensity to stand for themselves (not let gas lighting affect them) x
	# take chance p to encounter a racist (probability based on memory) into account as well
	# proportion cutoff between those who stand-up for themselves and those who do not (during agent creation random value)
	#x*(1-p) # for y = [0,0,1,0], p = sum(y)/len(y)
	# keep track of how many of the past interactions were slurs (separate memory, same length as bidding memory, 1 for slur, 0 otherwise)
	# on the other hand, one might argue that does with backbone/moral fibre/principles will not let themselves be affected, the more
	# they are slurred at---on the contrary, it might strengthen their resolve even more to stand up for themselves
	# x is the exponent in affected = random.uniform(0,1)**x, how much agents are affected by slurs
	x1 = 1 # sensibility exponent for group 1
	x2 = 1 # sensibility exponent for group 2
	x3 = 1 # sensibility exponent for group 3
	...
	audience_size = N-2
	aud_str = 0.25# strength of audience scores on disagreement point updates
	...
	jury_hist = []# agent_i and agent_j
	jury_sorted = []# how non-racist blue, racist blue, and red jurors rate non-racist blue agents, racist blue agents, and red agents
	sim_id = 0
...
# random.uniform(a,b)**x uniform random float between a and b, x tips the balance either towards a when x<1 or towards be when x>1
...
def create_pop(par):
	...
	for x in range(0,int(par.N1)):# blue agents
		...
		affected1 = random.uniform(0,1)**par.x1
		agents.append([a,[par.L,par.M,par.H][random.randint(0,2)],[par.L,par.M,par.H][random.randint(0,2)],[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],1,moves,[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],r,pop,moves,affected1])
	for z in range(0,int(par.N3)):# powerful individuals
		...
		pop = pop3s[z]
		moves = matrix(par.L,par.M,par.H,d1,d2)
		affected3 = random.uniform(0,1)**par.x3
		agents.append([a,[random.randint(0,2)],[random.randint(0,2)],[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],3,moves,[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],r,pop,moves,affected3])
	for y in range(0,int(par.N2)):# red agents
		...
		pop = pop2s[y]
		moves = matrix(par.L,par.M,par.H,d1,d2)
		affected2 = random.uniform(0,1)**par.x2
		agents.append([a,[random.randint(0,2)],[random.randint(0,2)],[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],2,moves,[[par.L,par.M,par.H][random.randint(0,2)]],[[par.L,par.M,par.H][random.randint(0,2)]],r,pop,moves,affected2])
	return(agents)
...
def belief_update(agent,update):# g1 = 1, g2 = 2
	group = agent[0]
	L = agent[11][0][0][0][0]
	M = agent[11][0][1][0][0]
	H = agent[11][0][2][0][0]
	d1 = agent[11][2][2][2][0]# group1's disagreement point
	d2 = agent[11][2][2][2][1]# group2's disagreement point
	moves = agent[11]
	if group==1 and d1>0 and (d1+update)>0:
		moves = matrix(L,M,H,d1+update,d2)
	if group==2 and d2>0 and (d2+update)>0:
		moves = matrix(L,M,H,d1,d2+update)
	new_agent = agent[0:11]+[moves]+agent[12:len(agent)]
	return(new_agent)
...
def interact(par,agents):
	...
	# log for print-out
	agent_i_old_ds = str([agent_i[11][2][2][2][0],agent_i[11][2][2][2][1]])
	agent_j_old_ds = str([agent_j[11][2][2][2][0],agent_j[11][2][2][2][1]])
	...
	if par.slurring == 'before':
		if agent_i[0] == 1 and agent_j[0] == 2 and random.uniform(0,1) < agent_i[9]:
			agent_j = belief_update(agent_j,-par.slur_str * agent_j[12])# slur strength mitigated by how much agent_j is affected by it
			slur_print = ' slur'
		elif agent_i[0] == 2 and agent_j[0] == 1 and random.uniform(0,1) < agent_j[9]:
			agent_i = belief_update(agent_i,-par.slur_str * agent_i[12])# slur strength mitigated by how much agent_i is affected by it
			slur_print = ' slur'

	# retrieve agents' move strategies
	if agent_i[0] == 1 and agent_j[0] == 1:
		moves_i = agent_i[11][0]
		moves_j = agent_j[11][0]
	elif agent_i[0] == 2 and agent_j[0] == 2:
		moves_i = agent_i[11][1]
		moves_j = agent_j[11][1]
	elif agent_i[0] == 1 and agent_j[0] == 2:
		moves_i = agent_i[11][2]
		moves_j = agent_j[11][2]
	elif agent_i[0] == 2 and agent_j[0] == 1:
		moves_i = agent_i[11][3]
		moves_j = agent_j[11][3]
	...
	agent_i_slur_ds = str([agent_i[11][2][2][2][0],agent_i[11][2][2][2][1]])
	agent_j_slur_ds = str([agent_j[11][2][2][2][0],agent_j[11][2][2][2][1]])
	...
	# calculate using actual disagreement points
	if agent_i[5] == 1:
	...
	if par.slurring == 'after':
		if agent_i[0] == 1 and agent_j[0] == 2 and random.uniform(0,1) < agent_i[9]:
			agent_j = belief_update(agent_j,-par.slur_str * agent_j[12])
			slur_print = ' slur'
		elif agent_i[0] == 2 and agent_j[0] == 1 and random.uniform(0,1) < agent_j[9]:
			agent_i = belief_update(agent_i,-par.slur_str * agent_i[12])
			slur_print = ' slur'
	...
	# A slurring event is different from a regular interaction.
	# Since the slurring hasn't actually taken bargaining power from the red agent, there's no need to increase it.
	# However, the audience should punish the racist behaviour.
	if slur_print == '':
		...
		agent_i = disagreement_update(agent_i,agent_i[9]*audience_score(agent_i,agent_j,audience,agent_i_move,agent_j_move,par)[0])
		agent_j = disagreement_update(agent_j,agent_j[9]*audience_score(agent_i,agent_j,audience,agent_i_move,agent_j_move,par)[1])

	# Record audience score for each agent
	...
	# log for print-out
	agent_i_new_ds = str([agent_i[11][2][2][2][0],agent_i[11][2][2][2][1]])
	agent_j_new_ds = str([agent_j[11][2][2][2][0],agent_j[11][2][2][2][1]])
...
def audience_score(agent_i,agent_j,audience,agent_i_move,agent_j_move,par):
		i_ratings, j_ratings = [], []
		jury = [[agent_i[0],agent_i[9],agent_j[0],agent_j[9]]]# first element contains group/racism info about agent_i and agent_j
		jury_sort = []
		for a in audience:
			...
			#a[0]# audience member's group membership
			#a[9]# audience member's propensity for racism
			#
			#if a[0] == 1 and a[9] == par.r1:# non-racist blue juror a[5] == 1
			# interaction round, juror's group (blue/red), racism index, type id, how affected juror is by slurs, disagreement point, believed disagreement point, and how they rate blue (type id = 1), red (2), and racist agents (3)
			jury_sort.append([par.r,a[0],a[9],a[5],a[12],a[6][2][2][2][0],a[11][2][2][2][0],'NA','NA','NA',par.sim_id,'strat1','strat2'])
			jury_sort.append([par.r,a[0],a[9],a[5],a[12],a[6][2][2][2][0],a[11][2][2][2][0],'NA','NA','NA',par.sim_id,'strat1','strat2'])
			#
			if agent_i[0] == 1 and agent_i[9] == par.r1:# agent_i[5] == 1 non-racist blue
				jury_sort[-2][7] = par.aud_str * e_i_rating
			elif agent_i[0] == 2 and agent_i[9] == par.r2:# agent_i[5] == 2 red
				jury_sort[-2][8]= par.aud_str * e_i_rating
			elif agent_i[0] == 1 and agent_i[9] == par.r3:# agent_i[5] == 3 racist
				jury_sort[-2][9] = par.aud_str * e_i_rating
			#
			if agent_j[0] == 1 and agent_j[9] == par.r1:# agent_j[5] == 1 non-racist blue
				jury_sort[-1][7] = par.aud_str * e_j_rating
			elif agent_j[0] == 2 and agent_j[9] == par.r2:# agent_j[5] == 2 red
				jury_sort[-1][8] = par.aud_str * e_j_rating
			elif agent_j[0] == 1 and agent_j[9] == par.r3:# agent_j[5] == 3 racist
				jury_sort[-1][9] = par.aud_str * e_j_rating

				#[jury_sort[6][agent[5]-1]append(par.aud_str*e_i_rating)]
				#[sum([1 for x in par.resp_hist_g1 if x == i]) for i in [par.L,par.M,par.H]]
			#elif a[0] == 2 and a[9] == par.r2:# red juror a[5] == 2
			#elif a[0] == 1 and a[9] == par.r3:# racist blue juror a[5] == 3
			#
			jury.append([par.r,a[0],a[9],par.aud_str*e_i_rating,par.aud_str*e_j_rating])
			#
		weights = [member[10] for member in audience]# more popular audience members get a bigger say
		...
		#
		par.jury_hist.append(jury)
		par.jury_sorted += jury_sort
...
def simulate(par):# run one simulation, i.e. par.trials number of interactions amongs a population of agents
	...
	if par.print_rewards == True:
		...
		print('Best responses gr1|1','('+str(len(par.resp_hist_g1_g1))+'/'+str(len(par.resp_hist_g1_g1)+len(par.resp_hist_g2_g2)+len(par.resp_hist_g1_g2))+')',str([sum([1 for z in [y for x in par.resp_hist_g1_g1 for y in x] if z == i]) for i in [par.L,par.M,par.H]]),
		...
		if par.N3 > 0:
			print('Racist',str(np.median(par.resp_hist_g3)),'('+str(len(par.resp_hist_g3))+'/'+str(len(par.resp_hist_g1_g1)+len(par.resp_hist_g2_g2)+len(par.resp_hist_g1_g2))+')',str([sum([1 for x in par.resp_hist_g3 if x == i]) for i in [par.L,par.M,par.H]]),
			...
			print('Final beliefs: gr1',str(beliefs(par.agent_pop1)[0]),'('+str(sum(beliefs(par.agent_pop1)[0])/len(beliefs(par.agent_pop1)[0]))+','+str(np.median(beliefs(par.agent_pop1)[0]))+')','gr2',str(beliefs(par.agent_pop1)[1]),'('+str(sum(beliefs(par.agent_pop1)[1])/len(beliefs(par.agent_pop1)[1]))+','+str(np.median(beliefs(par.agent_pop1)[1]))+')','gr3',str(beliefs(par.agent_pop1)[2]),'('+str(sum(beliefs(par.agent_pop1)[2])/len(beliefs(par.agent_pop1)[2]))+','+str(np.median(beliefs(par.agent_pop1)[2]))+')')
		else:
			print('Final disagreement points: gr1',str(disagreement_points(par.agent_pop1)[0]),'('+str(sum(disagreement_points(par.agent_pop1)[0])/len(disagreement_points(par.agent_pop1)[0]))+','+str(np.median(disagreement_points(par.agent_pop1)[0]))+')','gr2',str(disagreement_points(par.agent_pop1)[1]),'('+str(sum(disagreement_points(par.agent_pop1)[1])/len(disagreement_points(par.agent_pop1)[1]))+','+str(np.median(disagreement_points(par.agent_pop1)[1]))+')')
			print('Final beliefs: gr1',str(beliefs(par.agent_pop1)[0]),'('+str(sum(beliefs(par.agent_pop1)[0])/len(beliefs(par.agent_pop1)[0]))+','+str(np.median(beliefs(par.agent_pop1)[0]))+')','gr2',str(beliefs(par.agent_pop1)[1]),'('+str(sum(beliefs(par.agent_pop1)[1])/len(beliefs(par.agent_pop1)[1]))+','+str(np.median(beliefs(par.agent_pop1)[1]))+')')
...
def disagreement_points(population):
	gr1, gr2, gr3 = [], [], []
	for a in population:
		if a[0]==1 and a[9]!=1:# if group1: non-racist blue
			gr1.append(a[6][2][2][2][0])
		elif a[0]==2 and a[9]!=1:# if group2: red
			gr2.append(a[6][2][2][2][1])
		else:# if group3: racist
			gr3.append(a[6][2][2][2][0])
	return(gr1,gr2,gr3)

def beliefs(population):
	gr1, gr2, gr3 = [], [], []
	for a in population:
		if a[0]==1 and a[9]!=1:# if group1: non-racist blue
			gr1.append(a[11][2][2][2][0])
		elif a[0]==2 and a[9]!=1:# if group2: red
			gr2.append(a[11][2][2][2][1])
		else:# if group3: racist
			gr3.append(a[11][2][2][2][0])
	return(gr1,gr2,gr3)

def multi_sim(par,number,filename,jury_filename,print_results,save_results):
	output = []
	for n in range(0,number):
		...
		output.append([par.N1,par.N2,par.N3,# group sizes
			...
			np.median(beliefs(par.agent_pop1)[0]),np.median(beliefs(par.agent_pop1)[1]),np.median(beliefs(par.agent_pop1)[2])# final beliefs about disagreement points
			])
		for i in range(0,len(par.jury_sorted)):
			if par.jury_sorted[i][10] == par.sim_id:
				par.jury_sorted[i][11] = int(np.median(par.resp_hist_g1[-10:]))
				par.jury_sorted[i][12] = int(np.median(par.resp_hist_g2[-10:]))
	print('Done!')
	if print_results == True:
		print('N1'+'\t'+'N2'+'\t'+'N3'+'\t'+'ini_d1'+'\t'+'ini_d2'+'\t'+'ini_d3'+'\t'+'sl_str'+'\t'+'aud_str'+'\t'+'strat1'+'\t'+'strat2'+'\t'+'fin_d1'+'\t'+'fin_d2'+'\t'+'fin_d3'+'\t'+'fin_b1'+'\t'+'fin_b2'+'\t'+'fin_b3')
		for i in output:
	...
	if save_results == True:
		header = ['N1','N2','N3','ini_d1','ini_d2','ini_d3','sl_str','aud_str','strat1','strat2','fin_d1','fin_d2','fin_d3','fin_b1','fin_b2','fin_b3']
		...
		#
		# interaction round, juror's group (blue/red), racism index, type id, how affected juror is by slurs, disagreement point, believed disagreement point, and how they rate blue (type id = 1), red (2), and racist agents (3)
		header1 = ['round','jgr','jrac','jtype','jaff','jd','jbd','blue','red','racist','sim_id','strat1','strat2']
		with open('csv/'+str(jury_filename)+'.csv', 'wt') as f:
		    csv_writer = csv.writer(f,delimiter=',')
		    csv_writer.writerow(header1) # write header
		    for row in par.jury_sorted:
		    	csv_writer.writerow(row)
	#return(output)
